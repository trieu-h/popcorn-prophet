<script lang="ts">
  ...
  let last_movie_id_state: number | null = $state(null);
  ...
  async function on_input_change() {
    ...
    timeout = setTimeout(async () => {
      await get_movies(search_string.trim());
      is_loading = false;
    }, cooldown);
  }
  ...
  async function get_movies(title: string, last_movie_id?: number) {
    ...
    if (last_movie_id) {
      movies = [...movies, ...movies_json]
    } else {
      movies = movies_json;
    }

    if (movies.length) {
      last_movie_id_state = movies[movies.length - 1].id;
    }
  }

  window.addEventListener("scroll", () => {
    if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight) {
      get_movies(search_string, last_movie_id_state!);
    }
  })
